...
shinyServer(function(input, output, session){
  ...
  output$plot2 <- renderPlotly({
      ...
      yaxislabel = switch(ylabel,
                          "num_regulations" = "Number of regulations", 
                          "incidents_per_pop" = "Number of incidents per state population", 
                          "num_incidents" = "Number of incidents", 
                          "med_household_income" = "Median Household Income", 
                          "donald_trump" = "Vote Percentage: Donald Trump", 
                          "hillary_clinton" = "Vote Percentage: Hillary Clinton" 
  ...
  output$table_inc <- DT::renderDataTable({
    datatable(df_stat, rownames=FALSE, colnames = df_statcolnames) %>%
  ...
  output$table_mass <- DT::renderDataTable({
    datatable(massDT, rownames=FALSE, colnames = massDTcolumns, filter = 'top') %>%
      formatStyle(columns = colnames(massDT), background="skyblue", fontWeight='bold')
  ...
  # an attempt to use renderImage.  Not successful.
  # output$myimage <- renderImage({
  #   Mike_and_family <- tempfile(fileext = '.png')
  #   png(Mike_and_family, width = 800, height = 600)
  #   list(src = 'Mike_and_family',
